...
exports.Create = async (req: any, res: any) =>{
    try{
        ...
        if( NAME && DESCRIPTION && MAX_STUDENTS ){
          ...
          if(respOrm.err) return res.status(enum_.CODE_BAD_REQUEST).send(await magic.ResponseService(req,'Failure',respOrm.err.code,respOrm.err.messsage,''));
          return res.status(enum_.CODE_CREATED).send(await magic.ResponseService(req,'Success','','Curso creado exitosamente','')); 
        }
        return res.status(enum_.CODE_BAD_REQUEST).send(await magic.ResponseService(req,'Failure',enum_.ERROR_REQUIRED_FIELD,'Campos requeridos [NAME, DESCRIPTION, MAX_STUDENTS]',''));
    } catch(err) {
        console.log("err = ", err);
        return res.status(enum_.CODE_INTERNAL_SERVER_ERROR).send(await magic.ResponseService(req,'Failure',enum_.CRASH_LOGIC,'err',''));
    }
...
exports.UpdateById = async (req: any, res: any) =>{
    try{
        ...
        if( COURSE_ID && NAME && DESCRIPTION && MAX_STUDENTS ){
          ...
          if(respOrm.err) return res.status(enum_.CODE_BAD_REQUEST).send(await magic.ResponseService(req,'Failure',respOrm.err.code,respOrm.err.messsage,''));
          return res.status(enum_.CODE_CREATED).send(await magic.ResponseService(req,'Success','','Curso actualizado','')); 
        }
        return res.status(enum_.CODE_BAD_REQUEST).send(await magic.ResponseService(req,'Failure',enum_.ERROR_REQUIRED_FIELD,'Campos requeridos [NAME, DESCRIPTION, MAX_STUDENTS]',''));
    } catch(err) {
        console.log("err = ", err);
