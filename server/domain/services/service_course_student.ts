...
exports.Create = async (req: any, res: any) =>{
    ...
    try{
        ...
        if( Name && LastName && Age ){
            const respOrm = await ormCourseStudent.Store( Name, LastName, Age );
            if(respOrm.err){
                status = 'Failure', errorCode = respOrm.err.code, message = respOrm.err.messsage, statusCode = enum_.CODE_BAD_REQUEST;
            }else{
...
exports.UpdateById = async (req: any, res: any) =>{
    ...
    try{
        ...
        const Age = req.body.Age;
        if( Name && LastName && Age ){
            const respOrm = await ormCourseStudent.UpdateById( Name, LastName, Age, id );
            if(respOrm.err){
                status = 'Failure', errorCode = respOrm.err.code, message = respOrm.err.messsage, statusCode = enum_.CODE_BAD_REQUEST;
            }else{
...
exports.DeleteById = async (req: any, res: any) =>{
    ...
    try{
        const id = req.params.id;
            const respOrm = await ormCourseStudent.DeleteById(id);
            if(respOrm.err){
                status = 'Failure', errorCode = respOrm.err.code, message = respOrm.err.messsage, statusCode = enum_.CODE_BAD_REQUEST;
            }else{
